...
export class ShaderLoader {
  constructor() {
    ...
    if (saveButton) {
      saveButton.onclick = () => this.saveShader();
    }

    if (duplicateButton) {
      duplicateButton.onclick = () => this.duplicateCurrentShader();
    }
  ...
  async initEditor() {
    ...
    // Asegurarnos de que tenemos un shader por defecto
    if (!this.defaultShaders.fragment) {
      console.error('Los shaders por defecto no están cargados aún');
      return;
    }

    this.editor = new EditorView({
      doc: this.defaultShaders.fragment,
      extensions: [
        basicSetup,
        javascript(),
        oneDark,
  ...
  async initialize() {
    ...
    try {
      // 1. Cargar los shaders por defecto primero
      ...
      // 2. Inicializar con los shaders por defecto
      this.currentShaders = {
        vertex: this.defaultShaders.vertex,
        fragment: this.defaultShaders.fragment,
      };

      // 3. Ahora sí podemos inicializar el editor
      await this.initEditor();
      
      // 4. Configurar el resto de la UI
      this.setupButtons();
      this.setupTabListeners();
      await this.loadShaderStructure();
      this.setupUI();

      // 5. Inicializar desde URL si hay parámetros
      await this.initializeFromURL();

      // 6. Actualizar breadcrumb
  ...
  initializeModalElements() {
    ...
    if (this.chapterInput) {
      this.chapterInput.onchange = (e) => {
        this.newChapterInput.classList.toggle(
          "hidden",
          e.target.value !== "new"
        );
        this.updateExerciseInputOptions(e.target.value);
    ...
    if (this.exerciseInput) {
      this.exerciseInput.onchange = (e) => {
        this.newExerciseInput.classList.toggle(
          "hidden",
          e.target.value !== "new"
        );
      };
  ...
  async loadShaderStructure() {
    try {
      ...
      if (!result.success) {
        throw new Error(
          result.error || "Error al cargar la estructura de shaders"
        );
      }
  ...
  updateChapterSelect() {
    ...
    const sortedChapters = Array.from(this.chapters.entries()).sort((a, b) => {
    ...
    console.log(
      "Opciones finales en el selector:",
  ...
  updateExerciseSelect() {
    ...
    const exercises = Object.entries(chapterData.exercises || {}).sort(
      (a, b) => {
        const dateA = new Date(a[1].createdAt || 0);
        const dateB = new Date(b[1].createdAt || 0);
        return dateA - dateB;
  ...
  async loadShader(chapter, exercise) {
    ...
    try {
      ...
      const vertexResponse = await fetch(
        `/src/shaders/${chapter}/${exercise}/vertex.glsl`
      );
      const fragmentResponse = await fetch(
        `/src/shaders/${chapter}/${exercise}/fragment.glsl`
      );
      ...
      if (vertexResponse.ok) {
        ...
        const defaultVertexResponse = await fetch(
  ...
  async saveShader() {
    if (!this.currentChapter || !this.currentExercise) {
      alert(
        "Por favor, selecciona un capítulo y un ejercicio antes de guardar."
      );
      return;
    }
  ...
  updateSaveButtonVisibility() {
    ...
    if (this.currentChapter && this.currentExercise) {
      ...
      console.log(
        `Botón ${this.hasChanges ? "habilitado" : "deshabilitado"} porque ${
          this.hasChanges ? "hay" : "no hay"
        } cambios`
      );
    } else {
  ...
  updateURLParams() {
    ...
    if (!this.currentChapter) {
        ...
        // Sanitizar para la URL
        params.set("chapter", this.sanitizeId(this.currentChapter));
        
        if (this.currentExercise) {
            params.set("exercise", this.sanitizeId(this.currentExercise));
        } else {
            params.delete("exercise");
        }
    }
  ...
  updateExerciseInputOptions(chapterId) {
    ...
    // Convertir ejercicios a array y ordenar por fecha
    const sortedExercises = Object.entries(chapter.exercises).sort((a, b) => {
      const dateA = new Date(a[1].createdAt || 0);
      const dateB = new Date(b[1].createdAt || 0);
      return dateA - dateB;
    });
  ...
  async updateExerciseList() {
    try {
      ...
      const response = await fetch(
        `/api/list-exercises?chapter=${this.currentChapter}`
      );
      if (!response.ok) {
  ...
  async saveNewShader() {
    ...
    if (chapterInput.value === "new") {
        ...
        originalChapterName = newChapterInput.value.trim();
        finalChapterId = originalChapterName; // Ya no sanitizamos el ID de la carpeta
    } else {
        ...
        originalChapterName = finalChapterId;
    }
    ...
    originalExerciseName = newExerciseInput.value.trim();
    exerciseName = originalExerciseName; // Ya no sanitizamos el nombre del ejercicio
    ...
    // Usar los nombres originales para las rutas de los archivos
    ...
    try {
        ...
        // Actualizar el estado con los nombres originales
  ...
  async initializeFromURL() {
    ...
    if (urlChapter) {
        // Buscar el capítulo original que corresponde a la versión sanitizada de la URL
        const chapterSelect = document.getElementById("chapter-select");
        if (chapterSelect) {
            // Encontrar el capítulo original comparando las versiones sanitizadas
            const foundChapter = Array.from(this.chapters.entries()).find(([id, chapter]) => 
                this.sanitizeId(id) === urlChapter
            );

            if (foundChapter) {
                const [originalChapterId] = foundChapter;
                chapterSelect.value = originalChapterId;
                this.currentChapter = originalChapterId;
                ...
                if (urlExercise) {
                    const exerciseSelect = document.getElementById("exercise-select");
                    if (exerciseSelect) {
                        // Encontrar el ejercicio original comparando las versiones sanitizadas
                        const chapterData = this.chapters.get(originalChapterId);
                        const foundExercise = Object.entries(chapterData.exercises).find(([id, exercise]) => 
                            this.sanitizeId(id) === urlExercise
                        );

                        if (foundExercise) {
                            const [originalExerciseId] = foundExercise;
                            exerciseSelect.value = originalExerciseId;
                            this.currentExercise = originalExerciseId;

                            // Cargar el shader con los nombres originales
                            await this.loadShader(originalChapterId, originalExerciseId);
                        }
                    }
                }
            }
